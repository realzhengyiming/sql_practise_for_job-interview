...
+ 连续的排名不空缺（1，2，3，4）下面的解法，或者mysql8.0
  ...
  + **其中需要注意的是：over（partition by class order by property） 按照property排序进行累计，order by是个默认的开窗函数，按照class分区。这里面是按照class来进行不同类别的排序的**
    ...
    + rank()  over(patition by something order by other......)  这个是跳跃排序，相同的排1，1，然后下一个就是3
    + dense_rank() over(patition by something order by other......) 这种是连续的排序，1，1，1，1，2，2，3，3，3这样，适合这种。
  + 不使用窗口函数
    + 如下是其中的一种思路
+ 不连续的排名（1，1，3）这种
  + 
